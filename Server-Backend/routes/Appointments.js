...
router.delete('/availableAppointments/cancel', (req, res) => {

    const { id } = req.body;

    db.query('delete from AvailabilityList where id=?', [id],
...
router.get('/appointments', (req, res) => {
    ...
    db.query('select id, e.EmployeeID,e.name,Date,Hours from AvailabilityList,Employees e WHERE Date = ?', [date], (err, result) => {
        if (err) {
            console.log(err);
            res.send({ err: err });
        } else {
